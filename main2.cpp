...
int jal(string labelName, map<int, string>& labels, const vector<string> operands, map<int, int>& registers) {
   ...
   for (iter = labels.begin(); iter != labels.end(); iter++)
            {
                ...
                //  else if (iter == labels.end() || iter->second != labelName) {
    ...
    void ReadAndExecute() {
     ...
     while( entry!=instructions.end()) {
       // cout<<entry.first<<" da al first\n"<<entry.second<<endl;
        ...
        int nt=0;
        nt= jal(operands[2],labels,operands,registers);
        PC=nt+4;
